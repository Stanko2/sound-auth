...
int runSetup(Communication* c) {
    ...
    for (int tries = 0; tries < 3; tries ++) {
      ...
      ret = c->recv(7 + DH_KEY_SIZE);
      if (ret == 0)
        break;
    }

    if (ret == -1) {
      std::cout << "Maximum retries to setup a key exceeded, try again" << std::endl;
      return ret;
    }

    std::vector<uint8_t> v;
    ret = c->get_data(const_cast<std::vector<uint8_t>&>(v));
    std::cout << "Message: " << vectorToHexString(v) << std::endl;
    if (ret == 7 + DH_KEY_SIZE) {
        success = true;
