...
Waveforms::Waveforms(int buffer_size, int frame_size, WindowFunction* window_function) {
...
std::vector<float> Waveforms::createWaveform(
    const std::vector<float> &frequencies, const std::vector<float> &amplitudes,
    const std::vector<float> phases, int sample_rate, float duration) {
...
void Waveforms::addFilter(Filter *filter) { filters.push_back(filter); }
...
waveform Waveforms::getWaveform(const std::string data,
                                const int samples_per_frame,
                                const int sample_rate) {
  waveform out;
  float frame_duration =
      static_cast<float>(samples_per_frame) / static_cast<float>(sample_rate);
  ...
  for (auto &i : split(data, '|')) {
    ...
    waveform frame_waveform = createWaveform(frequencies, amplitudes, phases,
...
Spectrum *Waveforms::get_spectrum(const waveform &waveform, int sample_rate, float min_strength) {
  ...
  for (size_t i = 0; i < N; i++) {
    in[i][0] = window_function->apply(waveform[i], i);
    in[i][1] = 0;
  }
...
void Waveforms::enqueue_frame(const std::vector<float> &samples) {
  ...
  for(int i = 0; i < samples.size(); i++) {
    processed[i] = window_function->apply(processed[i], i);
  }
  recordFile.write(reinterpret_cast<const char *>(processed.data()),
                   len); // <- Pokazene
#endif
}
...
const float Spectrum::mag(const int f) {
  return sqrtf(data[f].real() * data[f].real() +
               data[f].imag() * data[f].imag());
}
